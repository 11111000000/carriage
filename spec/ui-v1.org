...
- C-c e f — открыть File chat (буфер для текущего файла; Intent=Ask; добавляет #+begin_context с абсолютным путём; GPTEL/visible контексты выключены).
...
- Переключатели (тумблеры в модлайне):
  ...
  - [Ctx] — carriage-mode-include-gptel-context (t/nil) — включать gptel-контекст (буферы/файлы; см. ./context-integration-v1.org). По умолчанию выключен.
  - [Files] — carriage-mode-include-doc-context (t/nil) — прикладывать файлы из блока #+begin_context документа (лимиты см. ./index.org).
  - [Vis] — carriage-mode-include-visible-context (t/nil) — включать видимые буферы текущего фрейма; по умолчанию OFF (см. ./context-integration-v1.org).
  - Все тумблеры — буферно-локальные; изменение влияет только на текущий carriage-буфер. реализация: переменные объявляются через make-variable-buffer-local, а команды-тумблеры используют setq-local.
...
- Событийная модель (LLM Transport → UI):  
  ...
  - Клавиша C-g (keyboard-quit) в любой момент должна отменять активный запрос/стрим: транспорт получает abort, прелоадер/спиннер останавливаются, UI переходит в состояние idle.
...
- При наличии carriage-doc-state: восстановить параметры и свернуть блок настроек (# +begin_carriage …) по умолчанию
  с помощью оверлейного механизма (carriage-block-fold), независимого от org-fold; блок автоматически раскрывается при
  входе курсора и снова скрывается при выходе; стандартные org-fold команды на него не влияют.
- Сворачивание блока begin_carriage после enable carriage-mode.  
- Выключение режима:
...
- Тултип [STATE] (help-echo):
  - Обязан показывать краткие подробности текущего состояния без тяжёлых вычислений на кадр.
  - Error — код ошибки и краткое сообщение (источник: transport|apply|engine).
  - Sending/Streaming/Reasoning — модель/провайдер (если есть), длительность, число чанков; для reasoning — хвост рассуждений (усечённый).
  - Dry-run/Apply/Done — сводка последней группы: ok/skip/fail/total.
  - Все строки через i18n-слой; длина ограничивается настройками.
  - TTY: команда carriage-show-state-details выводит текущие подробности в echo-area.
- Кликабельность:
