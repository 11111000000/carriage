...
- Поддержаны :op:
  ...
  - create/delete/rename/replace (./file-ops-v1.org)
...
* Применение
- Порядок типов: delete → rename → create → replace → patch → sre (см. ./apply-pipeline-v1.org).
...
- Совместимость Suite↔Engine: Suite='udiff требует 'git; Suite='sre допускает 'emacs|'git; при несовместимости → MODE_E_DISPATCH и мягкое предупреждение UI.
...
* Документация
- Обновлён ./index.org.
- README.org соответствует требованиям ./readme-v1.org.
