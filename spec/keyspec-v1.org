...
- Доступность глобальных клавиш:
  ...
  - Примечание: локальная привязка "C-c e" в carriage-mode всегда запускает меню; глобальный суффикс "e" (open-buffer) в таком буфере недостижим — это ожидаемо по семантике Emacs.
- Префикс C-c e:
...
(:id task-new     :cmd carriage-create-task-doc :keys ("n")    :contexts (carriage org global) :section tools :desc-key :task-new)
#+end_src
