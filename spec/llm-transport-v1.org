...
Addendum v1.2 — Traffic logging (structured, per-buffer, summaries)
- Adapters MUST log structured request/response to per-origin traffic buffers:
  - Buffer name: "*carriage-traffic:<basename>*".
  - Request format:
    - "==== BEGIN REQUEST (backend:model) ===="
    - "---- SYSTEM ----" (full system message, if present)
    - "---- PROMPT ----" (full prompt)
    - "---- CONTEXT FILES (content omitted) ----" (only paths/names)
    - "==== END REQUEST ===="
  - Response summary on done/abort:
    - "==== BEGIN RESPONSE (summary) ===="
    - "---- HEAD ----" first carriage-traffic-summary-head-bytes
    - "…"
    - "---- TAIL ----" last carriage-traffic-summary-tail-bytes
    - "==== END RESPONSE (bytes=K) ===="
- File logging option:
  - When carriage-traffic-log-to-file=t, adapters (or the transport layer) also append traffic lines to carriage-traffic.log.
- Capacity:
