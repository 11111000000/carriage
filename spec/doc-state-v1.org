...
- Каноническое хранение — блок настроек в начале файла:
  #+begin_example
  #+begin_carriage
  CARRIAGE_MODE t
  CARRIAGE_INTENT Code
  CARRIAGE_SUITE udiff
  CARRIAGE_MODEL_ID gptel:provider:model
  CARRIAGE_ENGINE git
  CARRIAGE_BRANCH_POLICY ephemeral
  CARRIAGE_CTX_GPTEL t
  CARRIAGE_CTX_DOC t
  CARRIAGE_REPORT_POLICY on-error
  CARRIAGE_STAGE_POLICY index
  #+end_carriage
  #+end_example
- Блок #+begin_carriage располагается в начале документа, скрыт по умолчанию (fold, overlay).
  Скрытие выполняется через оверлей (carriage-block-fold), независимо от org-fold:
  блок не раскрывается автоматически поиском/перепарсами; при входе курсора
  внутрь блока он временно раскрывается, при выходе — снова скрывается «заглушкой».
  Восстанавливается при включении режима и обновляется:
  - при изменении параметров (carriage-doc-state-sync-on-change=t по умолчанию);
  - перед сохранением файла (carriage-doc-state-save-on-save=t по умолчанию);
  - после сохранения (after-save) — повторное ensure со слегка отложенным запуском.
- Fallback для совместимости: допускаются строки #+PROPERTY: CARRIAGE_* и/или заголовок «Carriage State» с drawer; запись всегда производится в блок.
