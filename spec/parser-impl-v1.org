...
- Ошибки:
  ...
  - SRE_E_OCCUR_VALUE — значение :occur не 'first|'all.
  - SRE_E_EMPTY_SEGMENT — пустой FROM или TO.
...
;; v1.1: DELIM для :op \"create\" удалён; генерация токенов больше не используется.
;; Эта заглушка намеренно опущена в спецификации.
...
(defun carriage-parse-aibo (header body repo-root)
  "Парсинг AIBO v1 (literal-only FROM/TO). Запрещает :match и любые regex."
  )

(defun carriage-dry-run-aibo (plan-item repo-root)
  "Подсчитать совпадения (literal-only), подготовить мини-дифф и :changed-bytes."
  )

(defun carriage-apply-aibo (plan-item repo-root)
  "Применить literal-only пары; NOOP (after==before) → 'skip; опционально индексировать."
  )
...
(defun carriage-parse-replace (header body repo-root)
  "Парсинг replace: переписать содержимое существующего файла."
  )

(defun carriage-dry-run-replace (plan-item repo-root)
  "Симуляция replace: сравнить sizes/bytes и определить NOOP."
  )

(defun carriage-apply-replace (plan-item repo-root)
  "Атомарно перезаписать файл новым содержимым; ensure-final-newline при необходимости."
  )
