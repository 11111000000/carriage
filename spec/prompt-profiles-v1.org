...
- Шаги:
  ...
  4) guardrails ← статический текст правил:
     ...
     - DELIM — только для :op "create"; в SRE :delim запрещён. Закрывающий маркер должен быть РОВНО строкой вида ":DELIM" (двоеточие + токен) без пробелов и постфиксов на отдельной строке; строки вида ": DELIM", "::DELIM" или с хвостовыми пробелами запрещены (негативные примеры включать в prompt).
