...
- Переменные окружения (пути) предоставляет инструмент (Carriage). DELIM в v1 не используется; для :op "create" содержимое файла — это сырое тело блока между #+begin_patch и #+end_patch.
...
** Базовые сьюиты v1
- 'sre — разрешены :op (sre, aibo, create, replace, delete, rename).
- 'udiff — разрешены :op (patch, create, replace, delete, rename).
...
- Замечания:
  ...
  - Никогда не генерируй unified diff (:op "patch"); строки "--- a/" и "+++ b/" запрещены.
  - Ответ должен содержать ТОЛЬКО блоки begin_patch с :op "aibo" (в Intent=Code); в Hybrid допускается текст, но инструмент применяет только блоки.
  - Минимум одна пара FROM→TO обязателен. Пустой блок (без пар) — ошибка (SRE_E_SEGMENTS_COUNT).
  - Для полной замены содержимого файла НЕ используйте aibo — вместо этого применяйте :op "replace".
...
** FILE OPS (create/replace/delete/rename)
- create:
  - #+begin_patch (:version "1" :op "create" :file "RELATIVE/PATH")
  - СОДЕРЖИМОЕ ФАЙЛА
  - #+end_patch
- replace (переписать существующий файл целиком):
...
- Шаги:
  ...
  4) guardrails ← статический текст правил:
     ...
     - Suite=sre: разрешены только (sre aibo create delete rename) и никакого unified diff.
  - Suite=udiff: разрешены только (patch create delete rename) и никакого begin_from/begin_to.
  - AIBO (op): никогда не генерируй :op "patch"; при активном suite, который не включает 'patch, приёмщик должен отбрасывать такие блоки
