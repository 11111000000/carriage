...
- State tooltips:
  - При ошибке (симуляция через carriage-ui-note-error) help-echo у [STATE] содержит код и сообщение.
  - При стриме (эмуляция note-stream-progress/note-reasoning-chunk) help-echo обновляется по мере чанков и содержит длительность/чанки.
  - После dry-run/apply (эмуляция отчёта) help-echo содержит сводку ok/skip/fail/total.
- Контекст (тумблеры):
  - [Ctx] — переключает включение gptel-контекста (по умолчанию on).
  - [Files] — переключает включение файлов (#begin_context) (по умолчанию on).
- Бейдж [Ctx:N]:
  - Показывает количество элементов контекста; при выключенных обоих источниках — [Ctx:-].
  - help-echo содержит список всех элементов с источниками и причинами пропуска (если есть).
  - Проверка: число в бейдже совпадает с (length (:items …)) из сборки контекста, используемой при отправке.
- Инъекция контекста:
...
- Новые действия:
  ...
  - "C-c e M-d" → carriage-clear-patch-blocks (очистить все блоки patch в текущем документе).
