...
- Санитайзер/DELIM: перепрошивка маркеров выполняется одной функцией carriage-sre-rewrite-delim-markers из модуля lisp/carriage-sre-delim.el; регрессии: строки "<< DELIM" (с пробелом) не переписываются; ": DELIM"/"::DELIM" считаются ошибочными формами.
