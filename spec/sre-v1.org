...
* Addendum v1.1 — Надёжность (NOOP) и строгие значения :occur
- NOOP-политика: если симуляция даёт after==before, применение должно возвращать статус 'skip с указанием :matches и :changed-bytes=0 (а не 'ok).
- :occur — строго только 'first|'all; любые иные значения (включая числа) считаются ошибкой SRE_E_OCCUR_VALUE.
- Диагностика: отчёт обязан заполнять :matches для SRE; рекомендуется указывать :changed-bytes.
- Идемпотентные приёмы:
  - Якорение через \` (начало файла) и ^/$ (границы строк) в regex.
  - Терпимость к пробелам ([ \t]*) и ограничение :range для локальности.
  - «Охранные» проверки (guard) могут быть реализованы как опциональные расширения: :guard-next-line-not, :guard-prev-line-not, :guard-around (см. реализацию).
