...
- Область и границы
- Поддерживаемые форматы v1: sre, aibo, patch (unified diff), create, replace, delete, rename.
- В v1 движок 'emacs не применяет patch (unified diff); для patch обязателен 'git.
  Исключение в диспетчере: для 'emacs + 'patch возвращается дружелюбная строка результата
  со статусом 'skip (детали: "patch unsupported by emacs engine") вместо жёсткой ошибки.
- Default-движок: 'emacs.
...
- Порядок типов операций в группе: delete → rename → create → replace → patch → sre (см. ./apply-pipeline-v1.org).
...
- Suite='udiff → Engine='git (движок 'emacs не применяет patch).
- При несовместимости запуск dry-run/apply:
  - для пары 'emacs + 'patch — диспетчер возвращает строку результата со статусом 'skip
    и деталями "patch unsupported by emacs engine" (мягкая деградация);
  - для прочих несовместимостей — MODE_E_DISPATCH с подсказкой сменить движок (см. ./ui-v1.org).
...
- Поддерживает: sre, aibo, create, replace, delete, rename.
- Не поддерживает: patch (unified diff) — диспетчер возвращает дружелюбную строку результата
  со статусом 'skip и деталями "patch unsupported by emacs engine", чтобы избежать жёстких ошибок в отчёте.
...
- Поддерживает: patch, sre, aibo, create, replace, delete, rename.
- Стадирование:
...
- Apply:
  ...
  - replace: перезаписать файл новым содержимым; ensure-final-newline при t; индексировать при policy='index.
