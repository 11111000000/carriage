...
- sre:
  ...
  - apply: запись в рабочее дерево; опциональная индексация по policy; NOOP-политика — если after==before, вернуть 'skip и указать :matches и :changed-bytes=0.
- aibo:
  - dry-run: симуляция замен (literal-only); сбор мини-диффа.
  - apply: запись в рабочее дерево; опциональная индексация по policy; NOOP-политика аналогична sre (after==before → 'skip).
- file-ops:
...
- Синхронные обёртки:
  - Допускаются в прикладном слое как тонкие адаптеры над асинхронным движком (accept-process-output малыми интервалами), без изменения контракта движка. Рекомендуется async-first.
- Должны включать наблюдаемые метрики в отчёт (:pid, :elapsed-ms) для паритета с async.
...
- См. ./code-style-v1.org: async-first, безопасные синхронные обёртки и требование метрик (:pid, :elapsed-ms) в отчётах.
