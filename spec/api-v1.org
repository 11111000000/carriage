...
- carriage-apply-at-point-or-region (C-c e a)
  - Dry-run → подтверждение → применить блок =patch= под точкой; при активном регионе — применить все patch-блоки в регионе как группу.
- carriage-apply-last-iteration (C-c e A)
  - Применить все блоки последнего ответа. Строго: без метки — отказ.
...
- carriage-mode-headerline-show-outline (boolean) — t; показывать сегмент outline в header-line (можно отключить в больших Org-файлах).
- carriage-mode-use-icons (boolean) — t
- carriage-perf-cache-icons (boolean) — t; кэшировать иконки и доступность иконок для modeline/header-line.
- carriage-perf-cache-outline (boolean) — t; кэшировать строку outline в буфере (обновление вне hot-path).
- carriage-perf-cache-project (boolean) — t; кэшировать project root/name буферно-локально.
- carriage-ui-context-cache-ttl (number or nil) — 0.4; TTL кэша вычисления индикатора контекста (0 — отключить, nil — без ограничения).
...
** Дополнения v1.3 — Новые переменные и лица UI
- Новые переменные Customize:
  - carriage-mode-use-suite-icon (boolean) — default t; отображать Suite с иконкой вместо текста "Suite".
  - carriage-mode-use-engine-icon (boolean) — default t; отображать Engine с иконкой вместо текста "Engine".
  - carriage-mode-flash-patches (boolean) — default t; мигать «последними» патчами по завершении запроса.
  - carriage-mode-flash-duration (number) — default 1.0; длительность эффекта flash, сек.
  - carriage-mode-audio-notify (boolean) — default nil; включить звуковое уведомление о завершении.
  - carriage-mode-audio-sound (symbol|string) — 'beep или путь к звуковому файлу; используется когда audio-notify=t.
- Новые лица (faces) для состояния модлайна:
  - carriage-ui-state-idle-face — синий для idle.
  - carriage-ui-state-sending-face — зелёный для sending/streaming (спиннер наследует этот face).
  - carriage-ui-state-error-face — красный для error.

- Поведение выбора модели:
  - carriage-select-model — если активная модель равна "gptel-default", initial-input минибуфера пустой; иначе — предзаполняется текущим полным id backend[:provider]:model.
