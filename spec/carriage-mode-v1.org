...
- Входит:
  - Поддержка форматов v1: =sre= (./sre-v1.org), =aibo= (./aibo-v1.org) и =patch= (./patch-unified-diff-v1.org).
...
- Любой «шум» вне блоков игнорируется; неверные блоки маркируются предупреждениями.
  - «Последняя итерация» помечается идентификатором CARRIAGE_ITERATION_ID:
    ...
    - Генерация: новый UUIDv4 создаётся РОВНО один раз на запрос в carriage-transport-begin; команды отправки (send-buffer/send-subtree) не генерируют id.
...
- Внутригрупповый порядок:
  - delete → rename → create → replace → patch → sre.
  - Для одного файла: create/replace перед sre; rename перед sre (с новым путём); patch — перед sre, если оба присутствуют.
...
- Политика веток: 'in-place | 'wip (carriage/WIP по умолчанию) | 'ephemeral; dry-run не создаёт/не переключает ветки; переключение назад по завершении — по настройке.
...
- carriage-mode-default-intent ('Ask | 'Code | 'Hybrid) — по умолчанию 'Ask (чат)
