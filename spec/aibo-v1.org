...
- Минимум одна пара FROM→TO обязательна. Пустой блок (без пар) отклоняется с ошибкой SRE_E_SEGMENTS_COUNT.
- Для полного переписывания содержимого файла используйте :op "replace" (см. ./file-ops-v1.org); AIBO не предназначен для этого случая.
