...
* Элемент плана: REPLACE
- Каркас:
  - (:version "1" :op 'replace :file "path"
...
- Структура:
  - (:plan PLAN
     ...
     :branch-policy SYMBOL?        ; для 'git: 'in-place | 'wip | 'ephemeral
     :branch-name STRING?          ; для 'git: имя активной ветки применения (если отличается от исходной)
...
- Требования:
  ...
  - Для :op 'patch используется ключ :path; для остальных операций — :file.
