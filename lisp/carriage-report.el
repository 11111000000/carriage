...
REPORT shape:
  ...
  (let ((buf (carriage-report-buffer)))
    (with-current-buffer buf
      ...
      (carriage--report-insert-line '("#" "op" "path" "status" "matches" "details" "preview"))
      ...
      (let ((i 0))
        (dolist (it (or (plist-get report :items) '()))
          ...
          (let* ((op (plist-get it :op))
                 ...
                 (preview-short (if (and (stringp preview) (> (length preview) 0))
                                    (truncate-string-to-width preview 60 nil nil t)
                                  "")))
            (carriage--report-insert-line
             (list i op file status (or matches "") details preview-short)
             (pcase status
